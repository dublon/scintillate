function Answer=inputdlg_no_cancel(Prompt, Title, NumLines, DefAns, Resize)
%INPUTDLG_NO_CANCEL Input dialog box as per INPUTDLG but without a cancel
%box.

%  Function is built in to matlab and has been modified here according to
%  this discussion: https://www.mathworks.com/matlabcentral/newsreader/view_thread/257397
%
%  ANSWER = INPUTDLG_NO_CANCEL(PROMPT) creates a modal dialog box that returns user
%  input for multiple prompts in the cell array ANSWER. PROMPT is a cell
%  array containing the PROMPT strings.
%
%  INPUTDLG_NO_CANCEL uses UIWAIT to suspend execution until the user responds.
%
%  ANSWER = INPUTDLG_NO_CANCEL(PROMPT,NAME) specifies the title for the dialog.
%
%  ANSWER = INPUTDLG_NO_CANCEL(PROMPT,NAME,NUMLINES) specifies the number of lines for
...
%
%  ANSWER = INPUTDLG_NO_CANCEL(PROMPT,NAME,NUMLINES,DEFAULTANSWER) specifies the
...
%  strings.
%
%  ANSWER = INPUTDLG_NO_CANCEL(PROMPT,NAME,NUMLINES,DEFAULTANSWER,OPTIONS) specifies
...
%
%  answer=inputdlg_no_cancel(prompt,name,numlines,defaultanswer);
%
%  options.Resize='on';
%  options.WindowStyle='normal';
%  options.Interpreter='tex';
%
%  answer=inputdlg_no_cancel(prompt,name,numlines,defaultanswer,options);
%
%  See also DIALOG, ERRORDLG, HELPDLG, LISTDLG, MSGBOX,
%    QUESTDLG, TEXTWRAP, UIWAIT, WARNDLG .

%  inputdlg Copyright 1994-2007 The MathWorks, Inc.
%  inputdlg_no_cancel is simply the same function with the cancel button removed
%  $Revision: 1.58.4.13 $
